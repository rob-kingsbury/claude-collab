...
${isFinalRound ? `This is the FINAL exchange round. Focus on:
- Resolving any remaining disagreements — concede or provide final evidence
- Confirming which findings you both agree on
- Noting any unresolved points for the final report
- Do NOT repeat findings already agreed upon — just reference them` :
`- Respond to ${otherName}'s latest points — agree, disagree, or refine
- If ${otherName} flagged a false positive, either defend your finding with evidence or concede
- If ${otherName} raised new issues, verify them — read the code and confirm or challenge
- Add any new findings triggered by this exchange
- Push back where you have evidence — don't just agree to be agreeable
- Use your tools to verify claims before accepting or rejecting them`}

Be concise and substantive. Address specific points, cite file:line when relevant. Don't repeat yourself.
${personaBookend(persona)}`;
}
...
async function main() {
    ...
    try {
        ...
        // Fall back to raw conversation
        finalReport = `# Codebase Audit — Raw Exchange Log (synthesis failed)\n**Target**: ${targetDir}\n**Date**: ${new Date().toISOString().split('T')[0]}\n\n${conversationHistory}\n`;
    }
